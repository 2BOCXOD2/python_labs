...
class Queue:
    def __init__(self): # Deque реализован как двусвязный список. Внешне выглядит как обычный двусвязный список, но под капотом используется структура данных — связанный список блоков. Каждый блок — это массив указателей (например, на 64 элемента). При переполнении создаётся новый блок, связанный с предыдущим. Внутри каждого блока элементы хранятся как кольцевой буфер (индексы left и right двигаются циклически).
